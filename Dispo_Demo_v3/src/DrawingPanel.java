...
public class DrawingPanel extends JPanel {
    ...
    protected LinkedList<Position[]> pathsPink = new LinkedList<>();
    ...
    protected void paintComponent(Graphics g) {
        ...
        vertices.forEach(v -> {
            drawVertices(g2d, v, Color.BLACK);
        });
        ...
        paths.forEach(p -> {
            drawPath(g2d, p, Color.BLUE);
        });

        pathsPink.forEach(p -> {
            drawPath(g2d, p, Color.MAGENTA);
        });
    }
    ...
    void drawAuftrag(Graphics2D g2d, Auftrag v, Color c) {
        ...
        g2d.drawString(v.arrivalTime.toString(), x1, y1);
        g2d.drawOval(x2, y2, 10, 10);
        g2d.drawString(new Time(v.arrivalTime.calcTicks() + v.travelTime.calcTicks()).toString(), x2, y2);

        g2d.drawLine(x1, y1, x2, y2);
    }

    void drawVertices(Graphics2D g2d, Vertex v, Color c) {
    ...
    void drawFahrzeug(Graphics2D g2d, Fahrzeug v) {
        drawVertices(g2d, v, Color.BLUE);
    }

    void drawPath(Graphics2D g2d, Position[] pos, Color c) {
    ...
    void addPinkPath(Position[] pos) {
        this.pathsPink.add(pos);
    }
